...
// A client implementation of the NTLM authentication mechanism.
func NewNtlmClient(domain, username, password, workstation string) sasl.Client {
	return &ntlmAuth{
		Domain:      domain,
		UserName:    username,
		Password:    password,
...
func (n *ntlmAuth) Start() (string, []byte, error) {
	/*
		if !server.TLS {
			var isNTLM bool
			...
			b, err := n.InitialBytes()

			// dst := make([]byte, base64.StdEncoding.EncodedLen(len(b)))
			// base64.StdEncoding.Encode(dst, b)
			// fmt.Printf("AUTH NTLM %s", dst)
			// return "NTLM", nil, nil
			return "NTLM", b, err
		}

		return "", nil, errors.New("TLS not supported with NTLM")
	*/
	b, err := n.InitialBytes()
	return "NTLM", b, err
}

func (n *ntlmAuth) Next(fromServer []byte) ([]byte, error) {
	return n.NextBytes(fromServer)
	//b, err := n.NextBytes(fromServer)
	//dst := make([]byte, base64.StdEncoding.EncodedLen(len(b)))
	//base64.StdEncoding.Encode(dst, b)
	//fmt.Printf("3. %s\n", dst)
