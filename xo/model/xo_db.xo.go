...
// isNullFields return a bool value whether the field is set to null
func isNullFields(nullFields []string, field string) bool {
...
func GetRootSchema(extraQueries, extraMutations, extraTypes string) string {
	...
	type Query {
` +
		GetCountryQueries() + extraQueries +
		`}

type Mutation {
` +
		GetCountryMutations() + extraMutations +
		`}
...
func GetResolverAPIs(includeAPIs map[string]string, excludeAPIs []string) map[string]string {
	...
	for k, v := range GraphQLCountryAPIs {
		apis[k] = v
	}

	for k, v := range includeAPIs {
		apis[k] = v
	}
	for _, k := range excludeAPIs {
		delete(apis, k)
	}
