...
var ConditionFactories = map[string]func() Condition{
	...
	new(KeyValueCondition).GetName(): func() Condition {
...
func main() {
	{
		condition := &StringEqualCondition{Equals: "chengdu"}
		conditions := Conditions{"region": condition}

		jsonBuffer, err := conditions.MarshalJSON()
		fmt.Println("--->", string(jsonBuffer), err)

		cs := Conditions{}
		err = cs.UnmarshalJSON(jsonBuffer)
		fmt.Println("--->", err)
		for k, c := range cs {
			fmt.Println("\t", k, c)
		}
	...
	{
		condition := &KeyValueCondition{Key: "project", Value: "chengdu"}
		conditions := Conditions{"region": condition}

		jsonBuffer, err := conditions.MarshalJSON()
		//jsonBuffer, err := json.Marshal(conditions)
		fmt.Println("--->", string(jsonBuffer), err)

		//cs := Conditions{}
		err = cs.UnmarshalJSON(jsonBuffer)
		err = json.Unmarshal(jsonBuffer, &cs)
		fmt.Println("--->", err)
		for k, c := range cs {
			fmt.Println("\t", k, c)
		}
